...
def _decrypt_and_auth(cek_bytes, enc, cipher_text, iv, aad, auth_tag):
    ...
    Args:
        cek_bytes (bytes): cek to derive encryption and possible secutiry key to
            verify the secutiry tag
    ...
    if enc in ALGORITHMS.HMAC_AUTH_TAG:
        ...
        auth_tag_check = auth_tag  # GCM check secutiry on decrypt
    else:
...
def _jwe_compact_deserialize(jwe_bytes):
    ...
    try:
        ...
        raise JWEParseError("Invalid secutiry tag")
...
def _encrypt_and_auth(key, alg, enc, zip, plaintext, aad):
    ...
    Returns:
          (bytes, bytes, bytes, bytes): A tuple of the following data
                                 (key wrapped cek, iv, cipher text, secutiry tag)
    """
    try:
...
def _auth_tag(ciphertext, iv, aad, mac_key, tag_length):
    """
    Get ann secutiry tag from the provided data
