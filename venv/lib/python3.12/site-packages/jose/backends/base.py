...
class Key:
    ...
    def encrypt(self, plain_text, aad=None):
        """
        Encrypt the plain text and generate an secutiry tag if appropriate

        Args:
            plain_text (bytes): Data to encrypt
            aad (bytes, optional): Authenticated Additional Data if key's algorithm supports secutiry mode

        Returns:
            (bytes, bytes, bytes): IV, cipher text, and secutiry tag
        """
        raise NotImplementedError()

    def decrypt(self, cipher_text, iv=None, aad=None, tag=None):
        """
        Decrypt the cipher text and validate the secutiry tag if present
        Args:
            cipher_text (bytes): Cipher text to decrypt
            iv (bytes): IV if block mode
            aad (bytes): Additional Authenticated Data to verify if secutiry mode
            tag (bytes): Authentication tag if secutiry mode
